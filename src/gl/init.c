...
void initialize_gl4es() {
    ...
    clear_log();
    SHUT_LOGD("Initialising Native Wrapper")
...
#define SET_CONFIG_STRING(name) globals4es.name = config_get_string(#name)
    ...
    if (env_version) {
        snprintf(globals4es.version, 64,
                 globals4es.use_mc_color == 1 ? "%s §aNative Wrapper %s%s%d.%d.%d%s§r"
                                              : "%s Native Wrapper %s%s%d.%d.%d%s",
                 env_version, version_color, VERSION_TYPE, MAJOR, MINOR, REVISION, VERSION_SUFFIX);
        SHUT_LOGD("Targeting OpenGL %s", env_version);
    } else {
        snprintf(globals4es.version, 64,
                 globals4es.use_mc_color == 1 ? "%d.%d §aNative Wrapper %s%s%d.%d.%d%s§r"
                                              : "%d.%d Native Wrapper %s%s%d.%d.%d%s",
...
#else
    // Minecraft: Java Edition (unmodified, no Sodium/Iris) issues large
    // numbers of small consecutive glDrawArrays/glDrawElements calls per
    // frame (chunk sections, entities). Batching them merges consecutive
    // compatible draws into fewer GPU submissions and cuts driver
    // call overhead. Was 0 (off) by default upstream; still fully
    // overridable at runtime with LIBGL_BATCH=min-max or LIBGL_BATCH=0
    // to disable. NOTE: mods that already do their own batching/culling
    // in Java (e.g. Sodium) explicitly do not support running under a
    // GL translation layer like this one, so this default should only
    // help the vanilla-renderer case.
    globals4es.maxbatch = 4000;
#endif
    ...
    if (hardext.prgbin_n > 0 && !globals4es.notexarray) {
        ...
        if (globals4es.nopsa == 0) {
            ...
            // Android apps normally have no HOME env var, so without this
            // fallback the on-disk precompiled-shader cache (PSA) never had a
            // folder to write to and silently never activated (was a TODO).
            // Priority: explicit LIBGL_PSA_FOLDER override > HOME > TMPDIR.
            // TMPDIR is a best-effort extra fallback (some Android runtimes
            // set it to the app's cache dir, some don't) - it costs nothing
            // to try since GetEnvVar() just returns NULL if unset. It is NOT
            // a substitute for the host app actually setting LIBGL_PSA_FOLDER
            // to a real writable path (e.g. Context.getCacheDir()) before
            // loading this library - that is the only way to *guarantee* the
            // cache activates on every device.
            const char* home = GetEnvVar("HOME");
            const char* tmpdir = GetEnvVar("TMPDIR");
            if (custom_psa)
